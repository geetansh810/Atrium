...
**State: APPLIED through V3** (M0.1 + M0.2's `V2_1__seed.sql` + M0.7's `V3__budget_alerted_at.sql`, 2026-07-13). `V1__core.sql` (03 §V1 + 15 §1 ALTERs folded) and `V2__agent_platform.sql` (15 §§2–4) live in `core-api/src/main/resources/db/migration/` and apply clean on pgvector/pgvector:pg16 (compose + Testcontainers). `V3__budget_alerted_at.sql` (M0.7) adds `budgets.alerted_at TIMESTAMPTZ` — a small addendum 15 §1's `alert_pct` ALTER didn't anticipate needing until the enforcement code landed. **From now on: never edit V1/V2/V3 — they are applied.** [[frontend-shared]] `types.ts` already mirrors it.
