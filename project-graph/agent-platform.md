...
**State: FULL FOUNDATION LOOP LIVE** (2026-07-13, M0.3–M0.7). `eventbus/` DomainEvent + Topics (incl. `Topics.budget`, M0.7) + OutboxEvent entity + OutboxWriter (`MANDATORY` propagation — same-tx is structural; relay/cursors still M0.75). 13 §1 LLM provider SPI is built (`LlmProvider`/`LlmClient`/`LlmRouter`/`AnthropicClient`, M0.5a) and 13 §3 AgentRuntime SPI has its first real implementation (`LlmLoopRuntime`, M0.5b — replaces the M0.2 placeholder), now feedback-aware on rework (M0.6). The full task lifecycle is real end to end: create → claim → work → pending_review → approve/reject → (requeue) → budget-gated claim → auto-pause/resume (M0.6, M0.7). Still spec-only: `agentmind` (14 §§1–5: skills, memory, knowledge, learning pipeline — M-SK1 onward); outbox relay to Redis is still M0.75 (rows accumulate unpublished until then).
