...
**State: FULL FOUNDATION LOOP + EVENT RELAY LIVE** (2026-07-13, M0.3–M0.75). `eventbus/` DomainEvent + Topics (incl. `Topics.budget`, M0.7) + OutboxEvent entity + OutboxWriter (`MANDATORY` propagation — same-tx is structural) + `OutboxRelayGateway`/`JpaOutboxRelayGateway` (SKIP LOCKED claim + publish-stamp, same tx) + `EventCursorWorker` (durable-consumer base, no concrete consumer yet) + `OutboxRetentionJob` (nightly, 14-day/cursor-floor purge) — all M0.75. 13 §1 LLM provider SPI is built (`LlmProvider`/`LlmClient`/`LlmRouter`/`AnthropicClient`, M0.5a) and 13 §3 AgentRuntime SPI has its first real implementation (`LlmLoopRuntime`, M0.5b — replaces the M0.2 placeholder), now feedback-aware on rework (M0.6). The full task lifecycle is real end to end: create → claim → work → pending_review → approve/reject → (requeue) → budget-gated claim → auto-pause/resume (M0.6, M0.7) → **relayed to Redis for office/dashboard within ~250ms** (M0.75, [[realtimebridge]]). Still spec-only: `agentmind` (14 §§1–5: skills, memory, knowledge, learning pipeline — M-SK1 onward).
