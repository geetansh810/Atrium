...
- `mockData.ts` — typed loader over the JSON; each export maps 1:1 to a `04-api-contract` endpoint. Used by `mockStore.tsx` (`VITE_USE_MOCKS=1`); as of M2.5 `apiStore.tsx` no longer imports it (chat/announcements now come from the real API — only the activity feed has no backend endpoint and stays local).
- `api.ts`/`adapters.ts`/`queries.ts` (M0.8, extended every MF session since) — the real-API equivalent: typed fetch client, DTO adapters, React Query hooks. Consumed only by `apiStore.tsx`. `adaptAgent` now carries `paused` through (MF-4 fix); `usePatchAgentMutation` takes `{agentId, status?, paused?}`. **M2.5** added channels/messages/announcements/escalations fetchers + `adaptChannel`/`adaptMessage`/`adaptAnnouncement` + `useChannels`/`useChannelMessages`/`useSendMessageMutation`/`useAnnouncements`/`useAddAnnouncementMutation` — see [[communication]]/[[web-dashboard]].
