...
export default class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(
    ...
    // Atrium: status dot left of the name tag (position = status, dot = color)
    this.statusDot = this.scene.add.circle(0, 0, 3).setVisible(false);
    this.playerContainer.add(this.statusDot);
  ...
  setNameTag(name: string) {
    ...
    this.layoutStatusDot();
  }

  setStatusColor(color: number | undefined) {
    if (color === undefined) {
      this.statusDot.setVisible(false);
      return;
    }
    this.statusDot.setFillStyle(color).setVisible(true);
    this.layoutStatusDot();
  }

  private layoutStatusDot() {
    this.statusDot.setPosition(-this.playerName.width / 2 - 6, 0);
  }
