...
**Likely next:** M0.4 — Claim loop & lease (doc 17 card supersedes 07): canonical claim query (03) + two amendments — `attempt = attempt + 1` in SET, inner SELECT gains paused-agent NOT EXISTS guard — plus `BudgetGuard.canSpend` stub (true until M0.7) in the same service tx; `POST /tasks/{id}/claim` (X-Agent-Id) + `POST /tasks/{id}/lease/renew`; `LeaseReclaimJob` @Scheduled 60s under `pg_try_advisory_lock`, requeues via TaskEventRecorder; claim 409 body says who holds it; workers never retry a 409. Branch `m0.4-claim-loop`. Paste: 03 §claim query, 17 §M0.4 card, 04 (claim/renew), 05 §routing, 15 §1 semantics. Done-when: 3×20 exactly-once concurrency test; expired lease requeues ≤90s with visible event; attempt increments once per claim.
