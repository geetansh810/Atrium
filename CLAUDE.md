...
**Likely next:** M0.3 — Tasks, routing & event backbone (writer): `eventbus/` DomainEvent + OutboxWriter (same-tx), `routing/` create/list/get tasks + subtasks + pagination envelope, skill validated via AgentDirectory, TaskStateGuard (transitions matrix, wrong status→409), TaskEventRecorder (task_events + outbox in SAME tx, one helper), billing_task_id/request_depth on create. Branch `m0.3-routing`. Paste: 03 (tasks/subtasks/task_events), 04 §Tasks, 05 §routing, 12 §§3–4, 15 §2.
