...
**Build:** `ProcessRuntime` (or stub `EchoRuntime` if sandboxing is a rabbit hole — decide: EchoRuntime, completes tasks with canned artifact via TaskService like any worker) + fat-claim response (ContextBundle in claim body per 16 §5).
