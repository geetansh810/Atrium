...
| `V10__tenant_rls.sql` | 03 §V3's **RLS policies only** — `company_id = current_setting('app.company_id')::uuid` on every business table | M3.2 |
| `V11__compliance_gate.sql` | `role_definitions.review_required BOOLEAN NOT NULL DEFAULT false` (03 §V1 amendment) | M4.1 |
| `V12__seed_compliance_roles.sql` | 2 more global `role_definitions` (`legal`/`hr`, `review_required=true`, draft/research-labeling output contracts) | M4.1/M4.2 |
| `billing_accounts` (future, **unnumbered on purpose**) | 03 §V3's `billing_accounts` table | ⏸ M3.3 — deferred post-pilot (07 Rev D) |

**Split 2026-07-17 (07 Rev D — monetization deferred until after the pilot):** the old single `V10__multitenant_billing.sql` row bundled RLS with `billing_accounts`. M3.2 (RLS) is the next card up; M3.3 (Stripe) is now post-pilot backlog with no date — so bundling them would have forced M3.2's migration to create a table nothing reads for an indefinite stretch. RLS keeps V10; the billing table gets **no version number until M3.3 is actually picked up**, because by then this table's own rule (below) will have reassigned the intervening numbers anyway — guessing one now would just be another number to correct.
